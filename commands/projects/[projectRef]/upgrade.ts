...
export default Command(
  ...
  .Run(
    async (
      { Params, Log, Services },
    ): Promise<CommandStatus<UpgradeOutput>> => {
      ...
      try {
        ...
        if (Params.Json) {
          ...
          for (const [file, fileResults] of resultsByFile) {
            ...
            Log.Info(
              `  @${firstResult.oldVersion} → @${firstResult.newVersion}`,
            );
          ...
          if (Params.DryRun) {
            Log.Info(
              `📋 Dry-run complete: ${successCount} file(s) would be updated`,
            );
          } else if (failedCount > 0) {
            Log.Warn(
              `⚠️  Upgraded ${successCount} file(s), ${failedCount} failed`,
            );
          } else {
