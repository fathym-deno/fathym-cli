...
export default Command(
  ...
  .Run(
    async ({ Params, Log, Commands, Config, Services }): Promise<CommandStatus<ReleaseResult>> => {
      ...
      Log.Info('━'.repeat(50));
      Log.Info(`Step 1/2: Building and compiling for ${targets.length} targets`);
      Log.Info('━'.repeat(50));

      if (useAllTargets) {
        // Use --all flag for default targets (more efficient)
        await Compile([], { config: configPath, all: true, version: Params.Version });
      } else {
        // Loop through specific targets if user provided --targets
        for (const target of targets) {
          Log.Info(`\n🎯 Compiling for: ${target}`);
          await Compile([], { config: configPath, target, version: Params.Version });
        }
      }
      Log.Info('');
      ...
      return {
        ...
        Data: { cliName, version, targetCount: targets.length, scriptsGenerated },
      };
    },
  );
