...
const GitImportFlagsSchema = z.object({
  ...
  dir: z
    .string()
    .optional()
    .describe('Directory name for the bare clone (defaults to repository name)'),
  force: z
    .boolean()
    .optional()
...
export default Command(
  ...
  .Run(async ({ Services, Params, Log }): Promise<CommandStatus<GitImportResult>> => {
    ...
    Log.Info('');
    Log.Info(`Imported ${ctx.remote} → ${ctx.organization}/${ctx.repository}`);
    Log.Info('Next steps:');
    Log.Info('  - Verify the mirrored repository on GitHub');
    Log.Info('  - Run `ftm git configure -s` again if you need to adjust defaults.');
...
function buildTasks(): TaskDefinition<GitImportPipelineContext>[] {
  return [
    ...
    },
    {
      ...
      run: async (ctx, runtime) => {
        runtime.UpdateTitle(`Push mirror to ${ctx.organization}/${ctx.repository}`);
...
async function resolveOrganization(ctx: GitImportPipelineContext): Promise<string> {
  ...
  const answer = (await ctx.prompt.Input('Which GitHub organization should receive the mirror?'))
    .trim();
  if (answer) {
    return answer;
  }
...
async function resolveRepository(ctx: GitImportPipelineContext): Promise<string> {
  ...
  const answer = (await ctx.prompt.Input('Which GitHub repository should receive the mirror?'))
    .trim();
  if (answer) {
    return answer;
  }
...
function gitOptions(
  ctx: Pick<GitImportPipelineContext, 'cwd' | 'params'> & Partial<GitImportPipelineContext>,
): GitRunOptions {
  return {
    cwd: ctx.cwd,
