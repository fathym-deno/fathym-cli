...
Deno.test('upgradePackageReferences - filters by source type', async () => {
  const dfs = await createTestDFS({
    ...
    '/projects/app/src/main.deps.ts': `export * from 'jsr:${TEST_PKG}@${TEST_PKG_V1}';`,
  });
...
Deno.test('upgradePackageReferences - handles multiple references in same file', async () => {
  ...
  assertEquals(content.includes(`jsr:${TEST_PKG}@${TEST_PKG_V2}/handlers`), true);
...
Deno.test('upgradePackageReferences - preserves deep export barrel subpaths like /handlers/memory', async () => {
  ...
  const dfs = await createTestDFS({
    '/projects/app/deno.jsonc': JSON.stringify({
      ...
      exports: { '.': './mod.ts' },
      imports: {
        [`${TEST_PKG}/handlers/memory`]: `jsr:${TEST_PKG}@${TEST_PKG_V1}/handlers/memory`,
      },
    }),
  });
  ...
  assertEquals(content.includes(`jsr:${TEST_PKG}@${TEST_PKG_V1}/handlers/memory`), false);
});
...
Deno.test('findPackageReferences - only includes files within resolved projects', async () => {
  ...
  const dfs = await createTestDFS({
    ...
    '/standalone/utils.deps.ts': `export * from 'jsr:${TEST_PKG}@${TEST_PKG_V1}';`,
  });
  ...
  assertEquals(standaloneRef, undefined, 'Files outside projects should not be included');
...
Deno.test('findPackageReferences - projectName populated for nested files within project', async () => {
  ...
  const dfs = await createTestDFS({
    ...
    '/projects/cli/src/commands/build.deps.ts': `export * from 'jsr:${TEST_PKG}@${TEST_PKG_V1}';`,
    '/projects/cli/templates/init.hbs': `"${TEST_PKG}": "jsr:${TEST_PKG}@${TEST_PKG_V1}"`,
  });
