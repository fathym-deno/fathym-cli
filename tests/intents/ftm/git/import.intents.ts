...
CommandIntentSuite('git import Command Suite', cmd, GitIntentTestCLI)
  ...
  .Intent('prompts for inputs and supports dry-run depth/branch', (int) => {
    ...
    return int
      .Flags({ depth: 1, branch: 'integration', dir: 'import-tmp', 'dry-run': true })
      .WithServices({
  ...
  .Intent('allows --force to bypass configure gate and dry-run mirror', (int) => {
    const git = new MockGitService();

    return int
      .Args(['openindustrial', 'platform', 'https://github.com/example/platform.git'])
