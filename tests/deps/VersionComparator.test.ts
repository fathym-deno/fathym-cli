...
Deno.test('VersionComparator.isNewer - Switching from one channel to another', () => {
  ...
  assertEquals(comparator.isNewer('0.2.166-hmis', '0.2.165-integration'), false);
});
...
Deno.test('VersionComparator.hasChannel - Checks for specific channel', () => {
  assertEquals(comparator.hasChannel('0.2.299-integration', 'integration'), true);
...
Deno.test('VersionComparator.buildVersion - Builds channel version', () => {
  assertEquals(comparator.buildVersion('0.2.299', 'integration'), '0.2.299-integration');
