...
Deno.test('DepsFileParser.parse - Parses JSR package with channel version', () => {
  const content = `export { type EverythingAsCode } from "jsr:@fathym/eac@0.2.229-hmis";`;
  const refs = parser.parse(content);

  assertEquals(refs.length, 1);
  assertEquals(refs[0].registry, 'jsr');
  assertEquals(refs[0].fullName, '@fathym/eac');
  assertEquals(refs[0].version, '0.2.229-hmis');
...
export { b } from 'jsr:@fathym/eac@0.2.229-hmis';
`;
  const refs = parser.parse(content);
...
Deno.test('DepsFileParser.filterByPattern - Wildcard suffix match', () => {
  const content = `
export { a } from 'jsr:@fathym/eac@0.2.229-hmis';
export { b } from 'jsr:@fathym/eac-identity@0.0.74-eac-cascade';
