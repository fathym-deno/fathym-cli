...
├── my-cli                # Primary binary (from Tokens[0])
├── my-cli.exe            # (Windows only)
├── mc                    # Alias script (from Tokens[1])
└── mc.cmd                # (Windows only)
```
...
```
⚠️  Install path (~/.bin) is not in your PATH
👉 Add it to your shell profile to use CLI globally
```
